...
public class PrimeCalculatorV2 {
  ...
  public long calculatePrimes(long limit) {
    ...
    // --- Step 2: Parallel Segmented Sieving using Virtual Threads ---
    // For virtual threads, we don't explicitly determine 'numThreads' in the same
    // way,
    // as virtual threads are very lightweight and can be created in large numbers.
    // We'll create a task for each segment, and the virtual thread executor will
    // manage them.
    System.out.println("Using virtual threads for prime calculation.");

    // Create an ExecutorService that creates a new virtual thread for each
    // submitted task.
    // This is the idiomatic way to use virtual threads in modern Java.
    ExecutorService executor = Executors.newVirtualThreadPerTaskExecutor();
    List<Future<Void>> futures = new ArrayList<>(); // To hold results of submitted tasks

    // Determine a reasonable number of segments. Using the number of available
    // processors
    // as a heuristic for segment division is still sensible to create a good amount
    // of work units.
    int numSegments = Runtime.getRuntime().availableProcessors() * 2; // Can be adjusted
    if (numSegments == 0) {
      numSegments = 8; // Fallback
    }
    long segmentSize = limit / numSegments; // Approximate size of each segment

    // Submit tasks for each segment to the ExecutorService
    for (int i = 0; i < numSegments; i++) {
      long start = i * segmentSize;
      long end = (i == numSegments - 1) ? (limit + 1) : (i + 1) * segmentSize;
  ...
  public static void main(String[] args) {
    ...
    PrimeCalculatorV2 calculator = new PrimeCalculatorV2();
